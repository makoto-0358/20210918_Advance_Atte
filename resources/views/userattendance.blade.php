<x-appmember-layout>
    ...
    <!-- 打刻結果表示 -->
    <table class="mt-32 flex justify-center items-center md:w-4/5 md:mx-auto">
        ...
        @foreach($items as $item)
            <tr class="w-screen flex content-between border-t-2 border-gray-400 md:w-full">
                ...
                <td class="w-10 py-4 mx-auto flex justify-center items-center text-xs md:w-56 md:text-base">{{substr($item->start_time, 0, 10)}}</td>
                <td class="w-10 py-4 mx-auto flex justify-center items-center text-xs md:w-56 md:text-base">{{substr($item->start_time, 11, 8)}}</td>
                <td class="w-10 py-4 mx-auto flex justify-center items-center text-xs md:w-56 md:text-base">{{substr($item->end_time, 11, 8)}}</td>
                <td class="w-10 py-4 mx-auto flex justify-center items-center text-xs md:w-56 md:text-base">{{$item->sum_resting_time}}</td>
                <td class="w-10 py-4 mx-auto flex justify-center items-center text-xs md:w-56 md:text-base">{{$item->working_time}}</td>
            </tr>
        @endforeach
    </table>
